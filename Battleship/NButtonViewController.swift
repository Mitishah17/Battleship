...
class NButtonViewController: UIViewController {
    ...
    class NButtonViewController: UIViewController {
        ...
        required init?(coder aDecoder: NSCoder) {
            self.howManyButtons = 100
            self.loaded = false
            self.brain = BattleshipBrain(numButtonHit, numButtonMiss)
        ...
        override func viewDidLayoutSubviews() {
            if !loaded {
                setUpGameButtons(v: buttonContainer, totalButtons: self.howManyCards, buttonsPerRow: 5)
        ...
        func setUpGameLabel () {
            gameLabel.text = "Let's play Battleship!"
        }
